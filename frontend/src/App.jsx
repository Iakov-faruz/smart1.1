...
import ExpiryDiscounts from './components/ExpiryDiscounts';
import EditProductModal from './components/EditProductModal';
import AdminProductTable from './components/AdminProductTable'; // ייבוא הטבלה החדשה
import { getAllCategoriesWithProducts } from './api/get_all_categories_with_products';
import './App.css';
      ...
      {isCheckout ? (
        ...
        <>
          ...
          <div className="content-area" style={{ padding: '0 20px' }}>
            {/* אם המשתמש הוא מנהל - מציגים טבלה, אחרת מציגים את הגריד הרגיל */}
            {admin ? (
              <AdminProductTable products={allData} />
            ) : (
              <MainContent 
                products={filteredProducts} 
                onAddToCart={addToCart} 
